...
def fill_stock_move_location_dest_id2(env):
    openupgrade.logged_query(
        env.cr,
        """
        WITH RECURSIVE sub AS (
...
def migrate(env, version=None):
    has_mig18_location_dest_id = openupgrade.column_exists(
        env.cr, "stock_move", "mig18_location_dest_id"
    )
    columns_copy = _columns_copy
    field_renames = list(_field_renames)
    new_columns = list(_new_columns)
    if has_mig18_location_dest_id:
        if not openupgrade.column_exists(env.cr, "stock_move", "location_final_id"):
            openupgrade.logged_query(
                env.cr,
                """
                ALTER TABLE stock_move
                RENAME COLUMN mig18_location_dest_id TO location_final_id
                """,
            )
    else:
        columns_copy = {
        ...
        new_columns.append(("stock.move", "location_dest_id", "many2one"))
    ...
    openupgrade.add_columns(env, new_columns)
    fill_product_template_is_storable(env)
    if not has_mig18_location_dest_id:
        fill_stock_move_location_dest_id(env)
    else:
        fill_stock_move_location_dest_id2(env)
